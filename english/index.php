...
function convert_jsonfeed_to_rss($content = NULL, $max = NULL)
{
    ...
    foreach ($jf['result'] as $item) {
        ...
        if ($tanggal > $latestDate) $latestDate = $tanggal;
    }
    ...
    $xmlFeed->channel->addChild("pubDate", $tanggal);
    $xmlFeed->channel->addChild("lastBuildDate", $tanggal);
...
$content = @file_get_contents("https://nhentai-api.glitch.me/api/search?query=english");
